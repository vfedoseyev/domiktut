...
<head>
    ...
    <title>Добрый день, эта подборка специально для Вас!</title>
...
<div class="container">
    <div class="collection-title">Добрый день, эта подборка специально для Вас!</div>
    <div class="collection-description">Если Вы хотите посмотреть все дома, доступные на сайте, нажмите <a class="orange-link-text" href="<?php echo get_category_link(6) ?>">«Каталог»</a> </div>
    <?php

    $post_ids = explode(',', rtrim($_GET['collectionIds'], ','));


    $myposts = new WP_Query(['numberposts' => '-1', 'post__in' => $post_ids, 'post_type'   => 'post', 'category' => '6']);
